...
def _settle_update_job(mod, timeout: float = 8.0):
    """Wait for the background worker, then reconcile Linux 'restarting' to 'done'."""
    thread = getattr(mod, "_job_thread", None)
    if thread is not None:
        thread.join(timeout=timeout)
    deadline = time.time() + timeout
    data = mod.load_update_state()
    while data.get("state") == "running" and time.time() < deadline:
        time.sleep(0.05)
        data = mod.load_update_state()
    if data.get("state") == "restarting":
        mod.reconcile_update_state_on_startup()
        data = mod.load_update_state()
    return data
...
def test_start_update_job_streams_and_completes(isolated_updater, monkeypatch):
    ...
    data = _settle_update_job(isolated_updater)
    assert data["state"] == "done"
