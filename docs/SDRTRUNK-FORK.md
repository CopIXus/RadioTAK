...
- `GeoEventJsonExporter` — `PlottableDecodeEvent` GPS as `sdr2tak.location.v1` and call / ANI / affiliation metadata as `sdr2tak.decode.v1` NDJSON to `127.0.0.1:29500` (system/site/NAC/WACN/RFSS, timeslot, structured ALGID/KID, optional MI; MDC-1200 / FleetSync / Tait IDs when aux decode is on)
...
```
https://github.com/CopIXus/sdrtrunk/releases/download/v0.6.2-radiotak.6/sdr-trunk-linux-aarch64-v0.6.2-radiotak.6.zip
```
...
git tag v0.6.2-radiotak.6
git push origin radiotak-exporters
git push origin v0.6.2-radiotak.6
```
